...
xgb_sub.to_csv('xgb_e11.csv', index=False, float_format="%.9f")
lr_sub = pd.DataFrame({'id': ID_label, 'target': test_pred_lr})
lr_sub.to_csv('lr_e11.csv', index=False, float_format="%.9f")
bg_sub = pd.DataFrame({'id': ID_label, 'target': test_pred_bg})
bg_sub.to_csv('bg_e11.csv', index=False, float_format="%.9f")
