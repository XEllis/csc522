...
train_features_1 = [ 'ps_ind_05_cat', \
                     'ps_ind_17_bin', \
                     'ps_car_13', \
                     'ps_car_12', \
                     'ps_car_07_cat', \
...
for i in range(1):
    ...
    for train_index, test_index in folds.split(x_train_1, y_train_1):
        ...
        x_train_tmp = pd.DataFrame(x_train_tmp, columns=col)
        x_train_1 = pd.DataFrame(x_train_1, columns=col)
...
xgb_sub.to_csv('xgb_e22.csv', index=False, float_format="%.9f")
lr_sub = pd.DataFrame({'id': ID_label, 'target': test_pred_lr})
lr_sub.to_csv('lr_e22.csv', index=False, float_format="%.9f")
bg_sub = pd.DataFrame({'id': ID_label, 'target': test_pred_bg})
bg_sub.to_csv('bg_e22.csv', index=False, float_format="%.9f")
